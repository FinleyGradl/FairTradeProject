...
/**
 * Serves store cover images straight off disk on every request.
 *
 * Why this exists: with `output: "standalone"`, Next.js only serves files
 * that were present in /public at build time. Cover images are written at
 * runtime (see src/lib/uploads.ts), so requesting them directly under
 * /uploads/stores/... 404s even though the file is really on disk. Route
