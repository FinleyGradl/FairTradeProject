// path: src/app/api/v1/stores/[slug]/product-suggestions/[id]/vote/route.ts
import { NextRequest, NextResponse } from "next/server";
import { auth } from "@/auth";
import { castProductSuggestionVote } from "@/lib/products";
import { productSuggestionVoteSchema } from "@/lib/validators/product";
...
export async function POST(
  ...
  const parsed = productSuggestionVoteSchema.safeParse(body);
  if (!parsed.success) {
    return NextResponse.json({ error: "Ungültige Eingabe." }, { status: 400 });
  }

  const result = await castProductSuggestionVote(id, session.user.id, parsed.data.vote);
