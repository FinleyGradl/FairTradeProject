"use client";
// path: src/components/moderation/ReportedProductReviewsQueue.tsx
...
interface ReportedProductReview {
  id: string;
  rating: number;
  title: string | null;
  body: string;
  reportCount: number;
  product: { slug: string; name: string; store: { slug: string; name: string } };
  user: { name: string | null; email: string } | null;
  reports: { userName: string | null; reason: string | null; createdAt: string }[];
}

/** Mirrors ReportedReviewsQueue.tsx for store reviews. */
export function ReportedProductReviewsQueue({ reviews }: { reviews: ReportedProductReview[] }) {
  const router = useRouter();
  const [busyId, setBusyId] = useState<string | null>(null);
  const [items, setItems] = useState(reviews);

  async function hide(review: ReportedProductReview) {
    setBusyId(review.id);
    const res = await fetch(`/api/v1/product-reviews/${review.id}`, { method: "DELETE" });
    setBusyId(null);
    if (res.ok) {
  ...
  async function dismiss(review: ReportedProductReview) {
    setBusyId(review.id);
    const res = await fetch(`/api/v1/product-reviews/${review.id}/report`, { method: "DELETE" });
    setBusyId(null);
    if (res.ok) {
      setItems((prev) => prev.filter((r) => r.id !== review.id));
      router.refresh();
  ...
  if (items.length === 0) {
    return <p className="text-sm text-earth/60">Keine gemeldeten Produktbewertungen. 🎉</p>;
  }
      ...
      {items.map((review) => (
        <div key={review.id} className="rounded-xl border border-amber-200 dark:border-amber-800/40 bg-amber-50/40 dark:bg-amber-950/20 p-4">
          <div className="flex flex-wrap items-start justify-between gap-2">
            <div>
              <Link
                href={`/stores/${review.product.store.slug}/products/${review.product.slug}`}
                className="font-medium text-earth hover:underline"
              >
                {review.product.name} · {review.product.store.name}
              </Link>
