...
"use client";
// path: src/components/store/ReviewsList.tsx
...
export function ReviewsList({ reviews: initialReviews, isSignedIn, currentUserId }: ReviewsListProps) {
  const [reviews, setReviews] = useState(initialReviews);
  const [busyId, setBusyId] = useState<string | null>(null);
  const [error, setError] = useState<string | null>(null);

  async function handleReport(review: ReviewListItem) {
    setBusyId(review.id);
    setError(null);
    const res = await fetch(`/api/v1/reviews/${review.id}/report`, { method: "POST" });
  ...
  if (reviews.length === 0) {
    return <p className="text-sm text-earth/60">Noch keine Bewertungen. Sei die/der Erste!</p>;
  }
          ...
          <div key={review.id} className="rounded-lg border border-sage/10 bg-surface p-4">
            ...
            {review.ownerReply && (
              <div className="mt-3 rounded bg-sage-50 p-3 text-sm">
                <p className="font-medium text-sage dark:text-sage-300">Owner reply</p>
